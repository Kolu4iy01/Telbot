...
from shared import frequency_keyboard, stop_keyboard, start_keyboard
from shared import (is_valid_url, get_start_keyboard, get_duration_keyboard, save_users,
                    extract_domain, is_demo_limit_reached, users)
from shared import (active_sending, active_sessions, active_tasks, duration_options, logger,
                    user_durations, user_request_counter, user_urls, user_frequencies, frequency_options)
from shared import send_request_to_form
...
async def request_loop(user_id, frequency, url, state, message, duration=None):
    logger.info(f"Запуск request_loop для {user_id} з URL: {url}, частота: {frequency}, тривалість: {duration}")
